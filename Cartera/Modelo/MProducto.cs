...
namespace Cartera.Modelo
{
    class MProducto
    {
        ...
        {
            return Conexion.consulta("SELECT Id_Producto, Nombre_Producto as Producto, Numero_contrato as Contrato, Forma_pago as 'Forma Pago', Valor_Neto as 'Valor Neto', sum(Valor_Pagado) as Pagado,  Valor_Producto as 'Valor Final', Fecha_Venta as 'Fecha Venta', Observaciones, Proyecto_Nombre as Proyecto, Nom_Tipo_Producto as 'Tipo Producto', Fk_Id_Proyecto, Fk_Id_Tipo_Producto FROM Cliente_Producto INNER JOIN Producto on Id_Producto=Pfk_ID_Producto INNER JOIN Pagos on Fk_Id_Producto= Id_Producto INNER join Proyecto on Id_Proyecto = Fk_Id_Proyecto INNER join Tipo_Producto on Id_Tipo_Producto = Fk_Id_Tipo_Producto WHERE Pfk_ID_Cliente =" + IdCliente + " AND Estado_Cliente ='Activo' GROUP BY Id_Producto");
