    ...
    class MPago:MError
    {
        ...
        internal static int RegistrarPago(string porcentaje, string numero_Cuota, string fecha_Pago, string Concepto, string Entidad, string referencia_Pago, double valor_Pagado, string descuento, double valor_Descuento, string fk_Id_Producto)
        {
            ...
            catch (Exception ex)
            {
                ...
                LongError("RegistrarPago",sql, ex.Message);
            }
        ...
        internal static DataTable ListarPagosCliente(string productoid)
        {
            ...
            return Conexion.consulta("SELECT Id_Pagos, Numero_Cuota AS Cuota, Porcentaje AS Tipo, Concepto, Entidad, Valor_Pagado AS Valor, Fecha_Pago AS Fecha, Referencia_Pago AS Referencia, Descuento, Valor_Descuento AS 'Valor Descuento' FROM Pagos INNER JOIN Producto ON Fk_Id_Producto = Id_Producto INNER JOIN Financiacion ON Id_Producto = Fk_Producto WHERE Fk_Id_Producto = '" + productoid + "' AND Estado_Financiacion = 'Activa' ORDER BY Pagos.Porcentaje like 'Separación%' DESC, Pagos.Porcentaje like 'Inicial%' DESC,Pagos.Porcentaje like 'Saldo%' DESC, Fecha_Pago ASC,Numero_Cuota ASC");
        }
        internal static DataTable ReportesPagosCliente(string productoid)
        {
            return Conexion.consulta("SELECT  Numero_Cuota as 'Cuota', Porcentaje as 'Tipo', Concepto, Entidad, printf('% , d', Valor_Pagado) || substr(printf('%.2f', Valor_Pagado), instr(printf('%.2f', Valor_Pagado), '.'), length(printf('%.2f', Valor_Pagado)) - instr(printf('%.2f', Valor_Pagado), '.') + 1) as 'Valor', Fecha_Pago as Fecha, Referencia_Pago as Referencia, Descuento,printf('% , d', Valor_Descuento) || substr(printf('%.2f', Valor_Descuento), instr(printf('%.2f', Valor_Descuento), '.'), length(printf('%.2f', Valor_Descuento)) - instr(printf('%.2f', Valor_Descuento), '.') + 1) as  'Valor Descuento' FROM Pagos INNER JOIN Producto ON Fk_Id_Producto = Id_Producto INNER JOIN Financiacion ON Id_Producto = Fk_Producto WHERE Fk_Id_Producto = '" + productoid + "' AND Estado_Financiacion = 'Activa' ORDER BY Pagos.Porcentaje like 'Separación%' DESC, Pagos.Porcentaje like 'Inicial%' DESC,Pagos.Porcentaje like 'Saldo%' DESC, Fecha_Pago ASC,Numero_Cuota ASC");
        }

        internal static DataTable Tota_Recaudado_Producto(string productoid)
        {
            return Conexion.consulta("SELECT  Sum(Valor_Pagado) FROM Pagos INNER JOIN Producto ON Fk_Id_Producto = Id_Producto INNER JOIN Financiacion ON Id_Producto = Fk_Producto WHERE Fk_Id_Producto ='" + productoid + "' AND Estado_Financiacion = 'Activa';");
        }

        internal static DataTable SumarValorRecaudado(string productoid)
        {
            return Conexion.consulta("SELECT sum(Valor_Pagado) FROM Pagos INNER JOIN Producto ON Fk_Id_Producto = Id_Producto INNER JOIN Financiacion ON Id_Producto = Fk_Producto WHERE Fk_Id_Producto = '" + productoid + "' GROUP by Fk_Id_Producto=Fk_Id_Producto.");
        }
        internal static DataTable reportPagos(string FechaInicio, string FechaFin)
        {
            return Conexion.consulta("SELECT Nombre_Producto as Producto, Numero_Cuota as 'Cuota', Porcentaje as 'Tipo pago', Concepto, Fecha_Pago as Fecha, Referencia_Pago as Referencia, printf('%, d', Valor_Pagado)|| substr(printf('%.2f', Valor_Pagado), instr(printf('%.2f', Valor_Pagado), '.'), length(printf('%.2f', Valor_Pagado)) - instr(printf('%.2f', Valor_Pagado), '.') + 1) as Valor, Descuento, printf('%, d', Valor_Descuento) as'Valor Descuento' FROM Pagos INNER JOIN Producto on Id_Producto= Fk_Id_Producto INNER join Cliente_Producto on Pfk_ID_Producto=Id_Producto INNER JOIN Financiacion ON Id_Producto = Fk_Producto WHERE Estado_Cliente='Activo' and Fecha_Pago BETWEEN '" + FechaInicio + "' AND '" + FechaFin + "';");
        }
        internal static DataTable reportPagosTipo(string FechaInicio, string FechaFin, string Tipo)
        {
            return Conexion.consulta("SELECT Nombre_Producto as Producto, Numero_Cuota as 'Cuota', Porcentaje as 'Tipo pago', Concepto, Fecha_Pago as Fecha, Referencia_Pago as Referencia, printf('%, d', Valor_Pagado) as Valor, Descuento, printf('%, d', Valor_Descuento) as'Valor Descuento' FROM Pagos INNER JOIN Producto on Id_Producto= Fk_Id_Producto INNER join Cliente_Producto on Pfk_ID_Producto=Id_Producto INNER JOIN Financiacion ON Id_Producto = Fk_Producto   WHERE Estado_Cliente='Activo' AND Estado_Financiacion = 'Activa' and Fk_Id_Tipo_Producto = '" + Tipo + "' AND Fecha_Pago BETWEEN '" + FechaInicio + "' AND '" + FechaFin + "';");
        }                                                                                                                                                                                                                                                                                                                                                                                                                  
        internal static DataTable reportPagosProyecto(string FechaInicio, string FechaFin, string proyecto)                                                                                                                                                                                                                                                                                                                
        {                                                                                                                                                                                                                                                                                                                                                                                                                  
            return Conexion.consulta("SELECT Nombre_Producto as Producto, Numero_Cuota as 'Cuota', Porcentaje as 'Tipo pago', Concepto, Fecha_Pago as Fecha, Referencia_Pago as Referencia, printf('%, d', Valor_Pagado) as Valor, Descuento, printf('%, d', Valor_Descuento) as'Valor Descuento' FROM Pagos INNER JOIN Producto on Id_Producto= Fk_Id_Producto INNER join Cliente_Producto on Pfk_ID_Producto=Id_Producto INNER JOIN Financiacion ON Id_Producto = Fk_Producto  WHERE Estado_Cliente='Activo' AND Estado_Financiacion = 'Activa' and Fk_Id_Proyecto= '" + proyecto+"' AND Fecha_Pago BETWEEN '" + FechaInicio + "' AND '" + FechaFin + "';");
        }                                                                                                                                                                                                                                                                                                                                                                                                                  
        internal static DataTable reportPagosProyectoTipo(string FechaInicio, string FechaFin, string proyecto, string Tipo)                                                                                                                                                                                                                                                                                               
        {                                                                                                                                                                                                                                                                                                                                                                                                                  
            return Conexion.consulta("SELECT Nombre_Producto as Producto, Numero_Cuota as 'Cuota', Porcentaje as 'Tipo pago', Concepto, Fecha_Pago as Fecha, Referencia_Pago as Referencia, printf('%, d', Valor_Pagado) as Valor, Descuento, printf('%, d', Valor_Descuento) as'Valor Descuento' FROM Pagos INNER JOIN Producto on Id_Producto= Fk_Id_Producto INNER join Cliente_Producto on Pfk_ID_Producto=Id_Producto INNER JOIN Financiacion ON Id_Producto = Fk_Producto  WHERE Estado_Cliente='Activo' AND Estado_Financiacion = 'Activa' and Fk_Id_Proyecto= '" + proyecto + "' AND Fk_Id_Tipo_Producto = '" + Tipo + "' AND Fecha_Pago BETWEEN '" + FechaInicio + "' AND '" + FechaFin + "';");
        }
        internal static DataTable ValorReportPagos(string FechaInicio, string FechaFin)
        { 
            return Conexion.consulta("SELECT sum(Valor_Pagado) as valor, count(*) as pagos FROM Pagos INNER JOIN Producto on Id_Producto= Fk_Id_Producto INNER join Cliente_Producto on Pfk_ID_Producto=Id_Producto INNER JOIN Financiacion ON Id_Producto = Fk_Producto WHERE Estado_Cliente='Activo' AND Estado_Financiacion = 'Activa' and Fecha_Pago BETWEEN '" + FechaInicio + "' AND '" + FechaFin + "';");
        }
        ...
        {
            return Conexion.consulta("SELECT printf('%, d', SUM(Valor_Pagado)) as Valor, strftime('%m-%Y', Fecha_Pago) as 'Mes-Año' FROM Pagos INNER JOIN Producto ON Fk_Id_Producto = Id_Producto INNER JOIN Financiacion ON Id_Producto = Fk_Producto WHERE Estado_Financiacion = 'Activa' AND Fecha_Pago BETWEEN '" + FechaInicio + "' AND '" + FechaFin + "'group by strftime('%m-%Y', Fecha_Pago) ORDER BY Fecha_Pago;");
        }
        internal static DataTable PagosMesProyecto(string FechaInicio, string FechaFin, string proyecto)
        {
            return Conexion.consulta("SELECT printf('%, d', SUM(Valor_Pagado)) as Valor, strftime('%m-%Y', Fecha_Pago) as 'Mes-Año' FROM Pagos INNER JOIN Producto on Id_Producto= Fk_Id_Producto INNER JOIN Financiacion ON Id_Producto = Fk_Producto  WHERE Fk_Id_Proyecto= '" + proyecto + "' AND Estado_Financiacion = 'Activa' AND Fecha_Pago BETWEEN '" + FechaInicio + "' AND '" + FechaFin + "' group by strftime('%m-%Y', Fecha_Pago) ORDER BY Fecha_Pago;");
        }
    }
}
